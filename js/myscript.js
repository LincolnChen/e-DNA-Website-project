//edna page//

function show_building() {
  document.getElementById("info__layer").style.display = "none";
  document.getElementById("edna_info").style.display = "block";
}

function hide_building() {
  document.getElementById("info__layer").style.display = "block";
  document.getElementById("edna_info").style.display = "none";
}

function show_edna() {
  document.getElementById("edna__layer").style.display = "none";
  document.getElementById("edna_info2").style.display = "block";
}

function hide_edna() {
  document.getElementById("edna__layer").style.display = "block";
  document.getElementById("edna_info2").style.display = "none";
}


function show_edna1() {
  document.getElementById("edna2__layer").style.display = "none";
  document.getElementById("edna_info3").style.display = "block";
}

function hide_edna2() {
  document.getElementById("edna2__layer").style.display = "block";
  document.getElementById("edna_info3").style.display = "none";
}

function show_edna2() {
  document.getElementById("edna3__layer").style.display = "none";
  document.getElementById("edna_info4").style.display = "block";
}

function hide_edna3() {
  document.getElementById("edna3__layer").style.display = "block";
  document.getElementById("edna_info4").style.display = "none";
}

function show_edna3() {
  document.getElementById("edna4__layer").style.display = "none";
  document.getElementById("edna_info5").style.display = "block";
}

function hide_edna4() {
  document.getElementById("edna4__layer").style.display = "block";
  document.getElementById("edna_info5").style.display = "none";
}

function show_edna4() {
  document.getElementById("edna5__layer").style.display = "none";
  document.getElementById("edna_info6").style.display = "block";
}

function hide_edna5() {
  document.getElementById("edna5__layer").style.display = "block";
  document.getElementById("edna_info6").style.display = "none";
}

function show_edna5() {
  document.getElementById("edna6__layer").style.display = "none";
  document.getElementById("edna_info7").style.display = "block";
}

function hide_edna6() {
  document.getElementById("edna6__layer").style.display = "block";
  document.getElementById("edna_info7").style.display = "none";
}

function show_edna7() {
  document.getElementById("edna7__layer").style.display = "none";
  document.getElementById("edna_info8").style.display = "block";
}

//end edna page//




//gouldian starts//
function show_gouldian() {
  document.getElementById("gouldian__layer").style.display = "none";
  document.getElementById("gouldian_info").style.display = "block";
}

function hide_gouldian() {
  document.getElementById("gouldian__layer").style.display = "block";
  document.getElementById("gouldian_info").style.display = "none";
}

function show_gouldian2() {
  document.getElementById("gouldian2__layer").style.display = "none";
  document.getElementById("gouldian_info2").style.display = "block";
}

function hide_gouldian2() {
  document.getElementById("gouldian2__layer").style.display = "block";
  document.getElementById("gouldian_info2").style.display = "none";
}

function show_gouldian3() {
  document.getElementById("gouldian3__layer").style.display = "none";
  document.getElementById("gouldian_info3").style.display = "block";
}

function hide_gouldian3() {
  document.getElementById("gouldian3__layer").style.display = "block";
  document.getElementById("gouldian_info3").style.display = "none";
}
//gouldian ends//


//finch starts//
function show_finch() {
  document.getElementById("finch__layer").style.display = "none";
  document.getElementById("finch_info").style.display = "block";
}

function hide_finch() {
  document.getElementById("finch__layer").style.display = "block";
  document.getElementById("finch_info").style.display = "none";
}

function show_finch2() {
  document.getElementById("finch2__layer").style.display = "none";
  document.getElementById("finch_info2").style.display = "block";
}

function hide_finch2() {
  document.getElementById("finch2__layer").style.display = "block";
  document.getElementById("finch_info2").style.display = "none";
}

function show_finch3() {
  document.getElementById("finch3__layer").style.display = "none";
  document.getElementById("finch_info3").style.display = "block";
}

function hide_finch3() {
  document.getElementById("finch3__layer").style.display = "block";
  document.getElementById("finch_info3").style.display = "none";
}

function show_finch4() {
  document.getElementById("finch4__layer").style.display = "none";
  document.getElementById("finch_info4").style.display = "block";
}

function hide_finch4() {
  document.getElementById("finch4__layer").style.display = "block";
  document.getElementById("finch_info4").style.display = "none";
}

function show_finch5() {
  document.getElementById("finch5__layer").style.display = "none";
  document.getElementById("finch_info5").style.display = "block";
}

function hide_finch5() {
  document.getElementById("finch5__layer").style.display = "block";
  document.getElementById("finch_info5").style.display = "none";
}
//finch ends//

//conditions start//
function show_condition() {
  document.getElementById("condition__layer").style.display = "none";
  document.getElementById("condition_info").style.display = "block";
}

function hide_condition() {
  document.getElementById("condition__layer").style.display = "block";
  document.getElementById("condition_info").style.display = "none";
}

function show_condition2() {
  document.getElementById("condition2__layer").style.display = "none";
  document.getElementById("condition_info2").style.display = "block";
}

function hide_condition2() {
  document.getElementById("condition2__layer").style.display = "block";
  document.getElementById("condition_info2").style.display = "none";
}

function show_condition3() {
  document.getElementById("condition3__layer").style.display = "none";
  document.getElementById("condition_info3").style.display = "block";
}

function hide_condition3() {
  document.getElementById("condition3__layer").style.display = "block";
  document.getElementById("condition_info3").style.display = "none";
}

function show_condition4() {
  document.getElementById("condition4__layer").style.display = "none";
  document.getElementById("condition_info4").style.display = "block";
}

function hide_condition4() {
  document.getElementById("condition4__layer").style.display = "block";
  document.getElementById("condition_info4").style.display = "none";
}

function show_condition5() {
  document.getElementById("condition5__layer").style.display = "none";
  document.getElementById("condition_info5").style.display = "block";
}

function hide_condition5() {
  document.getElementById("condition5__layer").style.display = "block";
  document.getElementById("condition_info5").style.display = "none";
}

function show_condition6() {
  document.getElementById("condition6__layer").style.display = "none";
  document.getElementById("condition_info6").style.display = "block";
}

function hide_condition6() {
  document.getElementById("condition6__layer").style.display = "block";
  document.getElementById("condition_info6").style.display = "none";
}

function show_condition7() {
  document.getElementById("condition7__layer").style.display = "none";
  document.getElementById("condition_info7").style.display = "block";
}

function hide_condition7() {
  document.getElementById("condition7__layer").style.display = "block";
  document.getElementById("condition_info7").style.display = "none";
}

function show_condition8() {
  document.getElementById("condition8__layer").style.display = "none";
  document.getElementById("condition_info8").style.display = "block";
}

function hide_condition8() {
  document.getElementById("condition8__layer").style.display = "block";
  document.getElementById("condition_info8").style.display = "none";
}

function show_condition9() {
  document.getElementById("condition9__layer").style.display = "none";
  document.getElementById("condition_info9").style.display = "block";
}

function hide_condition9() {
  document.getElementById("condition9__layer").style.display = "block";
  document.getElementById("condition_info9").style.display = "none";
}

function show_condition10() {
  document.getElementById("condition10__layer").style.display = "none";
  document.getElementById("condition_info10").style.display = "block";
}

function hide_condition10() {
  document.getElementById("condition10__layer").style.display = "block";
  document.getElementById("condition_info10").style.display = "none";
}

function show_condition11() {
  document.getElementById("condition11__layer").style.display = "none";
  document.getElementById("condition_info11").style.display = "block";
}

function hide_condition11() {
  document.getElementById("condition11__layer").style.display = "block";
  document.getElementById("condition_info11").style.display = "none";
}

function show_condition12() {
  document.getElementById("condition12__layer").style.display = "none";
  document.getElementById("condition_info12").style.display = "block";
}

function hide_condition12() {
  document.getElementById("condition12__layer").style.display = "block";
  document.getElementById("condition_info12").style.display = "none";
}

function show_condition13() {
  document.getElementById("condition13__layer").style.display = "none";
  document.getElementById("condition_info13").style.display = "block";
}

function hide_condition13() {
  document.getElementById("condition13__layer").style.display = "block";
  document.getElementById("condition_info13").style.display = "none";
}

function show_condition14() {
  document.getElementById("condition14__layer").style.display = "none";
  document.getElementById("condition_info14").style.display = "block";
}

function hide_condition14() {
  document.getElementById("condition14__layer").style.display = "block";
  document.getElementById("condition_info14").style.display = "none";
}

function show_condition15() {
  document.getElementById("condition15__layer").style.display = "none";
  document.getElementById("condition_info15").style.display = "block";
}

function hide_condition15() {
  document.getElementById("condition15__layer").style.display = "block";
  document.getElementById("condition_info15").style.display = "none";
}

function show_condition16() {
  document.getElementById("condition16__layer").style.display = "none";
  document.getElementById("condition_info16").style.display = "block";
}

function hide_condition16() {
  document.getElementById("condition16__layer").style.display = "block";
  document.getElementById("condition_info16").style.display = "none";
}
//condition ends//

//waterhole1 start//


function show_waterhole1_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game_info1").style.display = "block";
}

function hide_waterhole1_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game_info1").style.display = "none";
}

function show_waterhole1_info1() {
  document.getElementById("waterhole1__layer").style.display = "none";
  document.getElementById("game_info2").style.display = "block";
}

function hide_waterhole1_info1() {
  document.getElementById("waterhole1__layer").style.display = "block";
  document.getElementById("game_info2").style.display = "none";
}

function show_waterhole1_info2() {
  document.getElementById("waterhole1__layer2").style.display = "none";
  document.getElementById("game_info3").style.display = "block";
}

function hide_waterhole1_info2() {
  document.getElementById("waterhole1__layer2").style.display = "block";
  document.getElementById("game_info3").style.display = "none";
}

function show_waterhole1_info3() {
  document.getElementById("waterhole1__layer3").style.display = "none";
  document.getElementById("game_info4").style.display = "block";
}

function hide_waterhole1_info3() {
  document.getElementById("waterhole1__layer3").style.display = "block";
  document.getElementById("game_info4").style.display = "none";
}

function show_waterhole1_info4() {
  document.getElementById("waterhole1__layer4").style.display = "none";
  document.getElementById("game_info5").style.display = "block";
}

function hide_waterhole1_info4() {
  document.getElementById("waterhole1__layer4").style.display = "block";
  document.getElementById("game_info5").style.display = "none";
}

//waterhole1 ends//

//waterhole2 start//
function show_waterhole2_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game2_info1").style.display = "block";
}

function hide_waterhole2_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game2_info1").style.display = "none";
}

function show_waterhole2_info1() {
  document.getElementById("waterhole2__layer").style.display = "none";
  document.getElementById("game2_info2").style.display = "block";
}

function hide_waterhole2_info1() {
  document.getElementById("waterhole2__layer").style.display = "block";
  document.getElementById("game2_info2").style.display = "none";
}

function show_waterhole2_info2() {
  document.getElementById("waterhole2__layer2").style.display = "none";
  document.getElementById("game2_info3").style.display = "block";
}

function hide_waterhole2_info2() {
  document.getElementById("waterhole2__layer2").style.display = "block";
  document.getElementById("game2_info3").style.display = "none";
}

function show_waterhole2_info3() {
  document.getElementById("waterhole2__layer3").style.display = "none";
  document.getElementById("game2_info4").style.display = "block";
}

function hide_waterhole2_info3() {
  document.getElementById("waterhole2__layer3").style.display = "block";
  document.getElementById("game2_info4").style.display = "none";
}

function show_waterhole2_info4() {
  document.getElementById("waterhole2__layer4").style.display = "none";
  document.getElementById("game2_info5").style.display = "block";
}

function hide_waterhole2_info4() {
  document.getElementById("waterhole2__layer4").style.display = "block";
  document.getElementById("game2_info5").style.display = "none";
}

//waterhole2 ends//

//waterhole3 start//
function show_waterhole3_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game3_info1").style.display = "block";
}

function hide_waterhole3_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game3_info1").style.display = "none";
}

function show_waterhole3_info1() {
  document.getElementById("waterhole3__layer").style.display = "none";
  document.getElementById("game3_info2").style.display = "block";
}

function hide_waterhole3_info1() {
  document.getElementById("waterhole3__layer").style.display = "block";
  document.getElementById("game3_info2").style.display = "none";
}

function show_waterhole3_info2() {
  document.getElementById("waterhole3__layer2").style.display = "none";
  document.getElementById("game3_info3").style.display = "block";
}

function hide_waterhole3_info2() {
  document.getElementById("waterhole3__layer2").style.display = "block";
  document.getElementById("game3_info3").style.display = "none";
}

function show_waterhole3_info3() {
  document.getElementById("waterhole3__layer3").style.display = "none";
  document.getElementById("game3_info4").style.display = "block";
}

function hide_waterhole3_info3() {
  document.getElementById("waterhole3__layer3").style.display = "block";
  document.getElementById("game3_info4").style.display = "none";
}

function show_waterhole3_info4() {
  document.getElementById("waterhole3__layer4").style.display = "none";
  document.getElementById("game3_info5").style.display = "block";
}

function hide_waterhole3_info4() {
  document.getElementById("waterhole3__layer4").style.display = "block";
  document.getElementById("game3_info5").style.display = "none";
}
//waterhole3 ends//

//waterhole4 start//
function show_waterhole4_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game4_info1").style.display = "block";
}

function hide_waterhole4_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game4_info1").style.display = "none";
}

function show_waterhole4_info1() {
  document.getElementById("waterhole4__layer").style.display = "none";
  document.getElementById("game4_info2").style.display = "block";
}

function hide_waterhole4_info1() {
  document.getElementById("waterhole4__layer").style.display = "block";
  document.getElementById("game4_info2").style.display = "none";
}

function show_waterhole4_info2() {
  document.getElementById("waterhole4__layer2").style.display = "none";
  document.getElementById("game4_info3").style.display = "block";
}

function hide_waterhole4_info2() {
  document.getElementById("waterhole4__layer2").style.display = "block";
  document.getElementById("game4_info3").style.display = "none";
}

function show_waterhole4_info3() {
  document.getElementById("waterhole4__layer3").style.display = "none";
  document.getElementById("game4_info4").style.display = "block";
}

function hide_waterhole4_info3() {
  document.getElementById("waterhole4__layer3").style.display = "block";
  document.getElementById("game4_info4").style.display = "none";
}

function show_waterhole4_info4() {
  document.getElementById("waterhole4__layer4").style.display = "none";
  document.getElementById("game4_info5").style.display = "block";
}

function hide_waterhole4_info4() {
  document.getElementById("waterhole4__layer4").style.display = "block";
  document.getElementById("game4_info5").style.display = "none";
}

//waterhole4 ends//

//waterhole5 start//
function show_waterhole5_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game5_info1").style.display = "block";
}

function hide_waterhole5_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game5_info1").style.display = "none";
}

function show_waterhole5_info1() {
  document.getElementById("waterhole5__layer").style.display = "none";
  document.getElementById("game5_info2").style.display = "block";
}

function hide_waterhole5_info1() {
  document.getElementById("waterhole5__layer").style.display = "block";
  document.getElementById("game5_info2").style.display = "none";
}

function show_waterhole5_info2() {
  document.getElementById("waterhole5__layer2").style.display = "none";
  document.getElementById("game5_info3").style.display = "block";
}

function hide_waterhole5_info2() {
  document.getElementById("waterhole5__layer2").style.display = "block";
  document.getElementById("game5_info3").style.display = "none";
}

function show_waterhole5_info3() {
  document.getElementById("waterhole5__layer3").style.display = "none";
  document.getElementById("game5_info4").style.display = "block";
}

function hide_waterhole5_info3() {
  document.getElementById("waterhole5__layer3").style.display = "block";
  document.getElementById("game5_info4").style.display = "none";
}

function show_waterhole5_info4() {
  document.getElementById("waterhole5__layer4").style.display = "none";
  document.getElementById("game5_info5").style.display = "block";
}

function hide_waterhole5_info4() {
  document.getElementById("waterhole5__layer4").style.display = "block";
  document.getElementById("game5_info5").style.display = "none";
}
//waterhole5 ends//

//waterhole6 start//
function show_waterhole6_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game6_info1").style.display = "block";
}

function hide_waterhole6_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game6_info1").style.display = "none";
}

function show_waterhole6_info1() {
  document.getElementById("waterhole6__layer").style.display = "none";
  document.getElementById("game6_info2").style.display = "block";
}

function hide_waterhole6_info1() {
  document.getElementById("waterhole6__layer").style.display = "block";
  document.getElementById("game6_info2").style.display = "none";
}

function show_waterhole6_info2() {
  document.getElementById("waterhole6__layer2").style.display = "none";
  document.getElementById("game6_info3").style.display = "block";
}

function hide_waterhole6_info2() {
  document.getElementById("waterhole6__layer2").style.display = "block";
  document.getElementById("game6_info3").style.display = "none";
}

function show_waterhole6_info3() {
  document.getElementById("waterhole6__layer3").style.display = "none";
  document.getElementById("game6_info4").style.display = "block";
}

function hide_waterhole6_info3() {
  document.getElementById("waterhole6__layer3").style.display = "block";
  document.getElementById("game6_info4").style.display = "none";
}

function show_waterhole6_info4() {
  document.getElementById("waterhole6__layer4").style.display = "none";
  document.getElementById("game6_info5").style.display = "block";
}

function hide_waterhole6_info4() {
  document.getElementById("waterhole6__layer4").style.display = "block";
  document.getElementById("game6_info5").style.display = "none";
}
//waterhole6 ends//

//waterhole7 start//
function show_waterhole7_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game7_info1").style.display = "block";
}

function hide_waterhole7_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game7_info1").style.display = "none";
}

function show_waterhole7_info1() {
  document.getElementById("waterhole7__layer").style.display = "none";
  document.getElementById("game7_info2").style.display = "block";
}

function hide_waterhole7_info1() {
  document.getElementById("waterhole7__layer").style.display = "block";
  document.getElementById("game7_info2").style.display = "none";
}

function show_waterhole7_info2() {
  document.getElementById("waterhole7__layer2").style.display = "none";
  document.getElementById("game7_info3").style.display = "block";
}

function hide_waterhole7_info2() {
  document.getElementById("waterhole7__layer2").style.display = "block";
  document.getElementById("game7_info3").style.display = "none";
}

function show_waterhole7_info3() {
  document.getElementById("waterhole7__layer3").style.display = "none";
  document.getElementById("game7_info4").style.display = "block";
}

function hide_waterhole7_info3() {
  document.getElementById("waterhole7__layer3").style.display = "block";
  document.getElementById("game7_info4").style.display = "none";
}

function show_waterhole7_info4() {
  document.getElementById("waterhole7__layer4").style.display = "none";
  document.getElementById("game7_info5").style.display = "block";
}

function hide_waterhole7_info4() {
  document.getElementById("waterhole7__layer4").style.display = "block";
  document.getElementById("game7_info5").style.display = "none";
}
//waterhole7 ends//

//waterhole8 start//
function show_waterhole8_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game8_info1").style.display = "block";
}

function hide_waterhole8_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game8_info1").style.display = "none";
}

function show_waterhole8_info1() {
  document.getElementById("waterhole8__layer").style.display = "none";
  document.getElementById("game8_info2").style.display = "block";
}

function hide_waterhole7_info1() {
  document.getElementById("waterhole7__layer").style.display = "block";
  document.getElementById("game7_info2").style.display = "none";
}

function show_waterhole8_info2() {
  document.getElementById("waterhole8__layer2").style.display = "none";
  document.getElementById("game8_info3").style.display = "block";
}

function hide_waterhole8_info2() {
  document.getElementById("waterhole8__layer2").style.display = "block";
  document.getElementById("game7_info3").style.display = "none";
}

function show_waterhole8_info3() {
  document.getElementById("waterhole8__layer3").style.display = "none";
  document.getElementById("game8_info4").style.display = "block";
}

function hide_waterhole8_info3() {
  document.getElementById("waterhole8__layer3").style.display = "block";
  document.getElementById("game8_info4").style.display = "none";
}

function show_waterhole8_info4() {
  document.getElementById("waterhole8__layer4").style.display = "none";
  document.getElementById("game8_info5").style.display = "block";
}

function hide_waterhole8_info4() {
  document.getElementById("waterhole8__layer4").style.display = "block";
  document.getElementById("game8_info5").style.display = "none";
}
//waterhole8 ends//

//waterhole9 start//
function show_waterhole9_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game9_info1").style.display = "block";
}

function hide_waterhole9_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game9_info1").style.display = "none";
}

function show_waterhole9_info1() {
  document.getElementById("waterhole9__layer").style.display = "none";
  document.getElementById("game9_info2").style.display = "block";
}

function hide_waterhole9_info1() {
  document.getElementById("waterhole9__layer").style.display = "block";
  document.getElementById("game9_info2").style.display = "none";
}

function show_waterhole9_info2() {
  document.getElementById("waterhole9__layer2").style.display = "none";
  document.getElementById("game9_info3").style.display = "block";
}

function hide_waterhole9_info2() {
  document.getElementById("waterhole9__layer2").style.display = "block";
  document.getElementById("game9_info3").style.display = "none";
}

function show_waterhole9_info3() {
  document.getElementById("waterhole9__layer3").style.display = "none";
  document.getElementById("game9_info4").style.display = "block";
}

function hide_waterhole9_info3() {
  document.getElementById("waterhole9__layer3").style.display = "block";
  document.getElementById("game9_info4").style.display = "none";
}

function show_waterhole9_info4() {
  document.getElementById("waterhole9__layer4").style.display = "none";
  document.getElementById("game9_info5").style.display = "block";
}

function hide_waterhole9_info4() {
  document.getElementById("waterhole9__layer4").style.display = "block";
  document.getElementById("game9_info5").style.display = "none";
}
//waterhole9 ends//

//waterhole10 start//
function show_waterhole10_info() {
  document.getElementById("game__layer").style.display = "none";
  document.getElementById("game10_info1").style.display = "block";
}

function hide_waterhole10_info() {
  document.getElementById("game__layer").style.display = "block";
  document.getElementById("game10_info1").style.display = "none";
}

function show_waterhole10_info1() {
  document.getElementById("waterhole10__layer").style.display = "none";
  document.getElementById("game10_info2").style.display = "block";
}

function hide_waterhole10_info1() {
  document.getElementById("waterhole10__layer").style.display = "block";
  document.getElementById("game10_info2").style.display = "none";
}

function show_waterhole10_info2() {
  document.getElementById("waterhole10__layer2").style.display = "none";
  document.getElementById("game10_info3").style.display = "block";
}

function hide_waterhole10_info2() {
  document.getElementById("waterhole10__layer2").style.display = "block";
  document.getElementById("game10_info3").style.display = "none";
}

function show_waterhole10_info3() {
  document.getElementById("waterhole10__layer3").style.display = "none";
  document.getElementById("game10_info4").style.display = "block";
}

function hide_waterhole10_info3() {
  document.getElementById("waterhole10__layer3").style.display = "block";
  document.getElementById("game10_info4").style.display = "none";
}

function show_waterhole10_info4() {
  document.getElementById("waterhole10__layer4").style.display = "none";
  document.getElementById("game10_info5").style.display = "block";
}

function hide_waterhole10_info4() {
  document.getElementById("waterhole10__layer4").style.display = "block";
  document.getElementById("game10_info5").style.display = "none";
}


function myFunction() {
  var x = document.getElementById("hcontent");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction2() {
  var x = document.getElementById("hcontent2");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction3() {
  var x = document.getElementById("hcontent3");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction4() {
  var x = document.getElementById("hcontent4");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction5() {
  var x = document.getElementById("hcontent5");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction6() {
  var x = document.getElementById("hcontent6");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction7() {
  var x = document.getElementById("hcontent7");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction8() {
  var x = document.getElementById("hcontent8");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction9() {
  var x = document.getElementById("hcontent9");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction10() {
  var x = document.getElementById("hcontent10");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction11() {
  var x = document.getElementById("hcontent11");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction12() {
  var x = document.getElementById("hcontent12");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction13() {
  var x = document.getElementById("hcontent13");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction14() {
  var x = document.getElementById("hcontent14");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction15() {
  var x = document.getElementById("hcontent15");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction16() {
  var x = document.getElementById("hcontent16");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction17() {
  var x = document.getElementById("hcontent17");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction18() {
  var x = document.getElementById("hcontent18");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}
function myFunction19() {
  var x = document.getElementById("hcontent19");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}

function myFunction20() {
  var x = document.getElementById("hcontent20");
  if (x.style.display === "block")
    {x.style.display = "none";}
   else {
    x.style.display = "block";
  }
}
